...
def getPeopleDataFromTheWeb(allPeopleInfoFolder, personAsProjectFolder, participantsIndex, ProjectsListwithSimilarities, ProjectsListwithSimilaritiesInProcessing):
    ...
    for webPerson, orbPerson in webPeopleToOrbPeopleIndexDict.iteritems():
        ...
        # print()
        curPretendPIndex = webPersonIndexToPretendIndex[webPerson]  #todo the webPersonIndex in webPersonIndexToPretendIndex and webPeopleToOrbPeopleIndexDict is different

        curPersonNameP = personIndexToFullNameInProcessing[orbPerson]

        aPersonPretendAsP = str(curPretendPIndex) + "\t" + curPersonNameP.encode('utf-8') + "\t" + curPersonNameP.encode('utf-8') + "\t" + "people"+"\t" + curPersonNameP.encode('utf-8') + "\t" + "Person" + "\t"+"1\t0\t0\t" + "People\t" + curPersonBio.encode('utf-8') + "\tNone Listedx" + "\tProjectIndex1" +"\tsimilarity1"
    ...
    with open(ProjectsListwithSimilaritiesInProcessing, "w+") as outTxtFinalProcessingFile:
