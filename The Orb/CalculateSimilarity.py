...
def generateProjectsListwithSimilarities(ProjectsListFile, stopwordsFile, ProjectsListwithSimilaritiesFile):
    ...
    with open(ProjectsListFile, "rU") as f:
    ...
    stopwords = set([line.strip().lower() for line in open(stopwordsFile)])
    ...
    # open output file
    f = open(ProjectsListwithSimilaritiesFile, "w")
    ...
    for index in range(1,len(Original_file)):
        ...
        for x in range(0,NumberOfSimilarities-1):
            #print "%s %s %s %s" % (index, x, str(all_similarities[index-1][x][0]), str(all_similarities[index-1][x][1]))
            print "%s %s" % (index, x)
