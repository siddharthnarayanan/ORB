...
def getPeopleDataFromTheWeb(allPeopleInfoFolder, personAsProjectFolder, participantsIndex, ProjectsListwithSimilarities, ProjectsListwithSimilaritiesInProcessing, dataImageFolderInProcessing, orbPeopleIndexToPretendProjectIndex):
    ...
    orbPeopleIndexToPretendProjectIndexDict = {}
    ...
    for aP in range(0, len(personalNameBioImageForAllWeb)):
        ...
        try:
            ...
            output = open(os.path.join(dataImageFolderInProcessing,imgName), 'wb')
    ...
    for webPerson, orbPerson in webPeopleToOrbPeopleIndexDict.iteritems():
        ...
        orbPeopleIndexToPretendProjectIndexDict[orbPerson] = curPretendPIndex
        ...
        for aOrbProjectIndex in aListOfAllProjectOfThePersion:
            aPersonPretendAsP10Default = aPersonPretendAsP10Default + "\t" + aOrbProjectIndex + "\t0.1"
    ...
    with open(orbPeopleIndexToPretendProjectIndex, "w+") as orbPeoToPro:

        orbPeoToPro.write("Peo\tPro")

        isFirst = True

        for aPerson, aProject in orbPeopleIndexToPretendProjectIndexDict.iteritems():

            # if isFirst is True:
            #     orbPeoToPro.write(str(aPerson) + "\t" + str(aProject))
            #     isFirst = False
            # else:
                orbPeoToPro.write("\n" + str(aPerson) + "\t" + str(aProject))
